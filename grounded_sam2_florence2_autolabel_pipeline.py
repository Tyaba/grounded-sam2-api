...
def caption_phrase_grounding_and_segmentation(
    ...
    # image caption
    caption_results = run_florence2(
        caption_task_prompt, None, florence2_model, florence2_processor, image
    )
    text_input = caption_results[caption_task_prompt]
    print(f'Image caption for "{image_path}": ', text_input)

    # phrase grounding
    grounding_results = run_florence2(
    ...
    cv2.imwrite(
        os.path.join(output_dir, "grounded_sam2_florence2_auto_labelling.jpg"),
        annotated_frame,
    )
    ...
    cv2.imwrite(
        os.path.join(
            output_dir, "grounded_sam2_florence2_auto_labelling_with_mask.jpg"
        ),
        annotated_frame,
    )
...
if __name__ == "__main__":
    ...
    if PIPELINE == "caption_to_phrase_grounding":
        ...
        raise NotImplementedError(
            f"Pipeline: {args.pipeline} is not implemented at this time"
        )
